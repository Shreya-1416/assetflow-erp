import { FiSearch } from "react-icons/fi";

function MaintenanceFilters({ onRaiseClick }) {
  return (
    <div className="flex flex-col md:flex-row justify-between gap-4">
      <div className="relative w-full md:w-96">
        <FiSearch className="absolute left-4 top-4 text-slate-400" />
        <input
          type="text"
          placeholder="Search maintenance request..."
          className="w-full rounded-lg border border-slate-300 py-3 pl-11 pr-4"
        />
      </div>

      <button onClick={onRaiseClick} className="rounded-lg bg-blue-600 px-6 py-3 font-semibold text-white hover:bg-blue-700">
        + Raise Request
      </button>
    </div>
  );
}

export default MaintenanceFilters;